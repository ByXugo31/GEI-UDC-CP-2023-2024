...
void displacements_elements(int sendcnts[], int disps[], int numprocs, int isScatter) {
    int i;
    for (i = 0; i < numprocs; i++) {
        sendcnts[i] = (N / numprocs);
        if (i < N % numprocs) sendcnts[i]++;
        if(isScatter) sendcnts[i] *= N;
        if (!i) disps[i] = 0;
        else disps[i] = disps[i - 1] + sendcnts[i - 1];
    }    
}
...
int main(int argc, char *argv[] ) {
    ...
    if(!rank){      
        ...
        displacements_elements(sendcounts,displs,numprocs,1);
    }
    ...
    displacements_elements(sendcounts,displs,numprocs,0);
