...
void *desplazamiento2(void *ptr){
    ...
    while (1) {
        ...
        pthread_mutex_lock(&n->mutex_decrements[min]);
        n->decrease[min]++;
        pthread_mutex_unlock(&n->mutex_decrements[min]);
        pthread_mutex_lock(&n->mutex_decrements[max]);
        n->decrease[max]--;
        pthread_mutex_unlock(&n->mutex_decrements[max]);
...
struct thread_info *start_threads(struct options opt, struct nums *nums, void *(func)(void *))
{
    int i;
...
void wait(struct options opt, struct nums *nums, struct thread_info *thrs, struct thread_info *thrs2, struct thread_info *thrs3){
    int i;
    for (i = 0; i < opt.num_threads; i++) {
        pthread_join(thrs[i].id, NULL);
        pthread_join(thrs2[i].id, NULL);
        pthread_join(thrs3[i].id, NULL);
    }
...
int main (int argc, char **argv)
{
    ...
    thrs = start_threads(opt, &nums, decrease_increase);
    thrs2 = start_threads(opt,&nums,desplazamiento);
    thrs3 = start_threads(opt,&nums,desplazamiento2);
    wait(opt, &nums, thrs, thrs2, thrs3);
